...
class SASRec(SequentialRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "BPR":
            ...
            user_emb = seq_output
            target_item_emb = test_item_emb[pos_items]
            in_batch_item = self.item_embedding(torch.unique(pos_items))

            self.batch_align = self.alignment(user_emb, target_item_emb)
            self.batch_uniform = (self.uniformity(user_emb) + self.uniformity(in_batch_item))/2
